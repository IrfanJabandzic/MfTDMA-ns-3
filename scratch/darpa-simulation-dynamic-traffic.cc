...
#define SIMULATION_DURATION 420.0 //in seconds
...
static uint64_t last_tx_timestamp = 0, last_ip_tx = 0;
#endif
...
#if defined(MEASURE_THROUGHPUT_ALL)
	uint64_t time = Simulator::Now ().GetMilliSeconds ();
	if(last_tx_timestamp + RX_TIMESTAMP_PERIOD <= time)
	{
		*stream2->GetStream () << time << "\t" << (ip_tx - last_ip_tx)*1000/(time - last_tx_timestamp) << std::endl;
		last_tx_timestamp = time;
		last_ip_tx = ip_tx;
	}
#endif
}
...
main (int argc, char *argv[])
{
  ...
  Config::SetDefault ("ns3::OnOffApplication::DataRate", StringValue ("900Bps"));
  //Config::SetDefault ("ns3::OnOffApplication::OnTime", StringValue ("ns3::ConstantRandomVariable[Constant=20]"));
  //Config::SetDefault ("ns3::OnOffApplication::OffTime", StringValue ("ns3::ConstantRandomVariable[Constant=10]"));

  Config::SetDefault ("ns3::OnOffApplication::OnTime", StringValue ("ns3::NormalRandomVariable[Mean=20|Variance=10|Bound=25]"));
  Config::SetDefault ("ns3::OnOffApplication::OffTime", StringValue ("ns3::NormalRandomVariable[Mean=15|Variance=6|Bound=18]"));
  ...
  UdpServerHelper udpServer (REMOTE_UDP_PORT);
	...
	{
		//PacketSinkHelper udpServer("ns3::UdpSocketFactory", InetSocketAddress (interfaces.GetAddress(node_count), REMOTE_UDP_PORT));
		ApplicationContainer serverApps = udpServer.Install (nodes);
		//ApplicationContainer serverApps = udpServer.Install (nodes.Get(node_count));
	...
	for(uint16_t node_count = 0; node_count < number_of_nodes; node_count++)
	{
			...
			{
				//udpClient.SetAttribute ("RemoteAddress", AddressValue(interfaces.GetAddress (0)));
				udpClient.SetAttribute ("Remote", AddressValue (Address(InetSocketAddress(interfaces.GetAddress (0), REMOTE_UDP_PORT))));
			}
			else
			{
				//udpClient.SetAttribute ("RemoteAddress", AddressValue(interfaces.GetAddress (node_count + 1)));
				udpClient.SetAttribute ("Remote", AddressValue (Address(InetSocketAddress(interfaces.GetAddress (node_count + 1), REMOTE_UDP_PORT))));
			}

			if(node_count < number_of_nodes/2)
			{
				udpClient.SetAttribute ("OnTime", StringValue ("ns3::ConstantRandomVariable[Constant=20]"));
				udpClient.SetAttribute ("OffTime", StringValue ("ns3::ConstantRandomVariable[Constant=20]"));
			}

			//udpClient.SetAttribute ("RemotePort", UintegerValue (REMOTE_UDP_PORT));
			//udpClient.SetAttribute ("MaxPackets", UintegerValue (number_of_packets));
			//udpClient.SetAttribute ("Interval", TimeValue (MicroSeconds (packet_period)));
			//udpClient.SetAttribute ("PacketSize", UintegerValue (PACKET_SIZE));

			clientApps[node_count] = udpClient.Install (nodes.Get(node_count));
  ...
  Ptr<OutputStreamWrapper> stream3 = asciiTraceHelper.CreateFileStream ("txII" + std::to_string(simulation_id) + ".data");
  //IP stats
  Config::ConnectWithoutContext ("/NodeList/*/$ns3::Ipv4L3Protocol/Drop", MakeCallback (&Ipv4L3ProtocolDropSink));
  Config::ConnectWithoutContext ("/NodeList/*/$ns3::Ipv4L3Protocol/Tx", MakeBoundCallback (&Ipv4L3ProtocolTxSink, stream3));
